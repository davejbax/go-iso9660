...
func TestBlockWriter_WriteBlock(t *testing.T) {
	...
	assert.Equal(t, 0x16*2048+testBlock16.Len(), buff.Len(), "WriteBlock should write all prior zeros and given block, and no more")
	...
	assert.Equal(t, 0x17*2048+len(testBlock17To18Data), buff.Len(), "WriteBlock should write correct number of bytes when given more than one block of data to write")
